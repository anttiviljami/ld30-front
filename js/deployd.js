...
(function (undefined) {
  ...
  if (!window._dpd) window._dpd = {};
  var hostname = 'hex.koodimonni.fi';
  var root = window.location.protocol + '//' + hostname;
...
dpd.connections = dpd("/connections");
dpd.connections.on = function(ev, fn) {
  return dpd.on("connections" + ":" + ev, fn);
}
dpd.connections.once = function(ev, fn) {
  return dpd.once("connections" + ":" + ev, fn);
}
dpd.connections.off = function(ev, fn) {
  return dpd.off("connections" + ":" + ev, fn);
}
dpd.connections.rename = function (path, body, fn) {
  dpd.connections.exec("rename", path, body, fn);
}

dpd.games = dpd("/games");
dpd.games.on = function(ev, fn) {
  return dpd.on("games" + ":" + ev, fn);
}
dpd.games.once = function(ev, fn) {
  return dpd.once("games" + ":" + ev, fn);
}
dpd.games.off = function(ev, fn) {
  return dpd.off("games" + ":" + ev, fn);
}
dpd.games.rename = function (path, body, fn) {
  dpd.games.exec("rename", path, body, fn);
}

dpd.hexes = dpd("/hexes");
dpd.hexes.on = function(ev, fn) {
  return dpd.on("hexes" + ":" + ev, fn);
}
dpd.hexes.once = function(ev, fn) {
  return dpd.once("hexes" + ":" + ev, fn);
}
dpd.hexes.off = function(ev, fn) {
  return dpd.off("hexes" + ":" + ev, fn);
}
dpd.hexes.rename = function (path, body, fn) {
  dpd.hexes.exec("rename", path, body, fn);
}

dpd.routenodes = dpd("/routenodes");
dpd.routenodes.on = function(ev, fn) {
  return dpd.on("routenodes" + ":" + ev, fn);
}
dpd.routenodes.once = function(ev, fn) {
  return dpd.once("routenodes" + ":" + ev, fn);
}
dpd.routenodes.off = function(ev, fn) {
  return dpd.off("routenodes" + ":" + ev, fn);
}
dpd.routenodes.rename = function (path, body, fn) {
  dpd.routenodes.exec("rename", path, body, fn);
}

dpd.teams = dpd("/teams");
dpd.teams.on = function(ev, fn) {
  return dpd.on("teams" + ":" + ev, fn);
}
dpd.teams.once = function(ev, fn) {
  return dpd.once("teams" + ":" + ev, fn);
}
dpd.teams.off = function(ev, fn) {
  return dpd.off("teams" + ":" + ev, fn);
}
dpd.teams.rename = function (path, body, fn) {
  dpd.teams.exec("rename", path, body, fn);
}
